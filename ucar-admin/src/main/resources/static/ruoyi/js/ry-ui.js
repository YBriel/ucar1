...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            // 回显部门信息
            selectCompayLabel: function(datas, value){
            	//console.info(value);
            	if(value == '*'){
            		return "不限";
            	}
            ...
            selectDeptLabel: function(datas, value){
                ...
                $.each(datas, function(index, dict) {
                    ...
                    if(words.length > 0){
                    	if (dict.deptId == words[0] || dict.deptId == words[1]){
                    		actions.push(dict.deptName);
                    		return;
                    	}
                    }
            ...
            //回显城市名称
            selectCityLabel: function(datas, value){
            	//console.info(value);
            	var confCode = $.common.trim(value);
            	var cityLabel = "";
                $.each(datas, function(index, dict) {
                	var dictCode = $.common.trim(dict.shortCode);
                    if (dictCode == confCode){
                    	cityLabel = dict.cityName;
                    	return false;
                    }
                });
                return cityLabel;
            ...
            //回显司机信息
            selectDriverNameLabel: function(datas, value){
            	//console.info(value);
            	var confCode = $.common.trim(value);
            	var cityLabel = "";
                $.each(datas, function(index, dict) {
                	var dictCode = $.common.trim(dict.userId);
                	//console.info(dictCode +"---"+confCode)
                    if (dictCode == confCode){
                    	cityLabel = dict.userName;
                    	return false;
                    }
                });
                return cityLabel;
            ...
            //回显司机类型
            selectDriverTypeLabel: function(datas, value){
            	//console.info(value);
            	var confCode = $.common.trim(value);
            	var cityLabel = "";
                $.each(datas, function(index, dict) {
          ...
          //回显CP名称
            selectCPLabel: function(datas, value){
            	//console.info(value);
            	var confCode = $.common.trim(value);
            	var cpLabel = "";
                $.each(datas, function(index, dict) {
                	var dictCode = $.common.trim(dict.id);
                	//console.info(dictCode + '---' + confCode);
                    if (dictCode == confCode){
                    	//console.info('--------------ok');
                    	cpLabel = dict.organizationName;
                    	return false;
                    }
                });
                return cpLabel;
          ...
          //回显cp fee rule名称
            selectCPFeeRuleLabel: function(datas, value){
            	console.info(value);
            	var id = $.common.trim(value);
            	var label = "";
                $.each(datas, function(index, dict) {
                	var dictId = $.common.trim(dict.id);
                	console.info(dictId + '---' + id);
                    if (dictId == id){
                    	//console.info('--------------ok');
                    	label = dict.ruleName;
                    	return false;
    	...
    	modal: {
            ...
            hidden: function() {
            	var doc = window.top == window.parent ? window.document : window.parent.document;
	        	$("a[class*=layui-layer-btn]", doc).attr("style", "display:none;");
            },
            // 启用按钮
            enable: function() {
            	var doc = window.top == window.parent ? window.document : window.parent.document;
